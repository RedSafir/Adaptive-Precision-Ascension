...
std::tuple<at::Tensor, at::Tensor> fused_linear_forward_cuda(
    ...
    if (bias_fwd.has_value() && pad_n == 0) {
        try {
            auto res = at::_scaled_mm(mm_x, mm_w, inv_scale_x, inv_scale_w, bias_fwd, /*use_fast_accum=*/false, target_dtype);
            out_2d = extract_scaled_mm_out(res);
            bias_added = true;
        } catch (...) {
            auto res = at::_scaled_mm(mm_x, mm_w, inv_scale_x, inv_scale_w, c10::nullopt, /*use_fast_accum=*/false, target_dtype);
            out_2d = extract_scaled_mm_out(res);
        }
    } else {
        auto res = at::_scaled_mm(mm_x, mm_w, inv_scale_x, inv_scale_w, c10::nullopt, /*use_fast_accum=*/false, target_dtype);
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> fused_linear_backward_cuda(
    ...
    if (needs_grad_input) {
        ...
        auto res_in = at::_scaled_mm(mm_g, mm_w, inv_scale_grad, inv_scale_w, c10::nullopt, false, act_dtype);
    ...
    if (needs_grad_weight) {
        ...
        auto res_w = at::_scaled_mm(mm_gt, mm_x, inv_scale_grad, inv_scale_x, c10::nullopt, false, at::kFloat);
